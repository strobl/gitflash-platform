...
interface UebungInterviewCardProps {
  image: string;
  title: string;
  description: string;
  duration: string;
...
export const UebungInterviewCard: React.FC<UebungInterviewCardProps> = ({
  image,
  title,
  description,
  duration,
  ...
  return (
    <article className="bg-white shadow-[0px_12px_32px_rgba(0,0,0,0.08)] w-full overflow-hidden rounded-xl cursor-pointer hover:shadow-lg transition-shadow">
      <img src={image} className="aspect-[1.8] object-cover w-full" alt={title} />
      <div className="flex w-full flex-col items-stretch px-3 py-4">
        <h3 className="text-[#0A2540] text-xl font-bold line-clamp-1">{title}</h3>
