...
export const UebungStartSection: React.FC<UebungStartSectionProps> = ({ 
  ...
  if (showCameraAccessUI) {
    return (
      <div className="bg-[#1A1F2C] text-white rounded-xl shadow-sm overflow-hidden h-full">
        <div className="flex items-center justify-center flex-col text-center py-16 px-6">
          ...
          <Button 
            ...
            className="bg-white text-[#0A2540] rounded-[100px] px-8 py-2.5 hover:bg-white/90 transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isRequesting ? (
              <>
                <div className="animate-spin h-4 w-4 border-2 border-[#0A2540]/20 border-t-[#0A2540] rounded-full mr-2 inline-block"></div>
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border p-8 h-full">
